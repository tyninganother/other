...
class CtaTemplate(object):
    ...
    def cancelAlllong(self):
        """全部撤单"""
        self.ctaEngine.cancelAlllong(self.name)
    # -------------------------------------------------------
    def cancelAllshort(self):
        """全部撤单"""
        self.ctaEngine.cancelAllshort(self.name)    
    ...
    #---------------------------------------------------------------------
    def getCtaPositionDetails(self):
        positionDetails = self.ctaEngine.mainEngine.getPositionDetail(self.vtSymbol)
        
        return  [positionDetails.longPosFrozen, positionDetails.shortPosFrozen]
...
class BarGenerator(object):
    ...
    def updateTick(self, tick):
        ...
        # 新的一分钟
            ...
            #print '形成1分钟Bar,lastTick为：\n',self.lastTick.__dict__
...
class ArrayManager(object):
    ...
    def __init__(self, size=3):
        ...
        #self.upordown = True          # True 代表笔向上   Fales  代表笔向下
        #self.fenxing = 0                       # 0 代表顶分型，1代表底分型，2代表向上笔，3代表向下笔        
    ...
    def updateBar(self, bar):
        ...
        if self.count == 1 :
            ...
            return
        # 第2根K线调用k线合并处理函数kxhb2
        elif self.count == 2:
             # 第2根K线H和L均大于第1根K线时，缓存第2根K线，并标记upordown为True，即笔向上
            if ( bar.high > self.highArray[-1] ) and ( bar.low > self.lowArray[-1] ):
                self.upordown = True
             # 第2根K线H和L均小于第1根K线时，缓存第2根K线，并标记upordown为False，即笔向下
            elif  ( bar.high < self.highArray[-1] ) and ( bar.low < self.lowArray[-1] ):
                self.upordown = False
             # 第2根K线和第1根K线存在包含关系时，使用第2根K线覆盖第1根，不修改upordown值，并修改count值为1
            else :
                self.highArray[-1] = bar.high
                self.lowArray[-1] = bar.low
                self.count -= 1
        ...
        # 第3根K线调用k线合并处理函数kxhb3,并返回最新3根k线的分型情况，即fenxing的值
        else:
            if  (bar.high > self.highArray[-1] ) and ( bar.low > self.lowArray[-1] ):
                if self.upordown :
                    self.fenxing = 2
                # 如果如果upordown为False，则前3根K线形成底分型，fenxing=1，并将upordown赋值为False
                else :
                    self.fenxing = 1
                    self.upordown = False
            # 第3根K线H和L均小于第2根K线时，缓存第3根K线，如果upordown为True，则前3根K线形成顶分型，fenxing=0，upordown=False
            elif ( bar.high < self.highArray[-1] ) and ( bar.low < self.lowArray[-1] ):
                if self.upordown :
                    self.fenxing = 0
                    self.upordown = False
                # 如果如果upordown为False，则前3根K线形成向下1笔，fenxing=3
                else :
                    self.fenxing = 3
            # 第3根K线和第2根K线存在包含时
            else :
                # 如果如果upordown为True，合并第2根和第3根K线时，取两根k线H、L中最大值为新K线的HL，fenxing值保持不变
                if self.upordown :
                    self.highArray[-1] = max(self.highArray[-1], bar.high)
                    self.lowArray[-1] = max(self.lowArray[-1], bar.low)

                # 如果如果upordown为False，合并第2根和第3根K线时，取两根k线H、L中最小值为新K线的HL，fenxing值保持不变
                else :
                    self.highArray[-1] = min(self.highArray[-1], bar.high)
